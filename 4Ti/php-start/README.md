...
2. Skopiuj cały ten folder do katalogu `htdocs`, tak żeby nazywał się `php-start`:
   - Windows: `C:\xampp\htdocs\php-start`
   - Mac: `/Applications/XAMPP/htdocs/php-start`
3. Wpisz w przeglądarce adres `http://localhost/php-start/`.
...
- **W przeglądarce widzisz kod PHP zamiast strony.** Plik został otwarty podwójnym kliknięciem,
  ...
  wejdź na `http://localhost/php-start/`.
...
- **Strona jest pusta.** Sprawdź, czy plik nazywa się dokładnie `index.php` i leży w `htdocs/php-start`.
